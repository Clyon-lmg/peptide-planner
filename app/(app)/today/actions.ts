...
export async function getTodayDosesWithUnits(dateISO: string): Promise<TodayDoseRow[]> {
    ...
    // 1. Fetch SINGLE Active Protocol (Logic from uploaded actions.ts)
    ...
    // 4. Inventory & Status
    // 🟢 LOOSE LOOKUP: We do NOT filter by protocol_id here, matching status.ts logic.
    ...
    const rows: TodayDoseRow[] = dayRows.map((dr) => {
        ...
        // Find the original item to get schedule details for forecast
        const originalItem = items.find((i:any) => Number(i.peptide_id) === pid);
...
async function upsertDoseStatus(peptide_id: number, dateISO: string, targetStatus: DoseStatus) {
    ...
    // Note: If no active protocol, we might fail to log if we require protocol_id. 
    // But per instructions, we stick to single protocol logic. 
    // However, for STATUS, we check existing record loosely.
    ...
    const { data: existing } = await sa
        ...
        .eq('user_id', user.id)
        // .eq('protocol_id', protocol.id) // 🟢 REMOVED to match status.ts
