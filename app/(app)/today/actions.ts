...
export async function getTodayDosesWithUnits(dateISO: string): Promise<TodayDoseRow[]> {
    // 🟢 CRITICAL FIX: Disable caching to ensure we see the status change immediately
    noStore();
    ...
    // 1. Get Active Protocol & Schedule
    ...
    // 2. Fetch Actual Doses (ALL doses for this date, ignoring protocol links)
    ...
    // 4. Generate Schedule Rows (Same logic as Calendar)
