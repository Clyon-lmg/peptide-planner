...
export async function getTodayDosesWithUnits(dateISO: string): Promise<TodayDoseRow[]> {
    ...
    // 🟢 CALENDAR LOGIC PORT: Iterate items and check isDoseDayUTC
    if (protocol?.id && items) {
        const d = new Date(dateISO + 'T00:00:00Z'); // Exact same date handling as Calendar
        const protocolStart = protocol.start_date || dateISO;

        for (const it of items) {
            const pid = Number(it.peptide_id);
            // Mimic ScheduleItem type for the engine
...
async function upsertDoseStatus(peptide_id: number, dateISO: string, targetStatus: DoseStatus) {
    ...
    // Check existing record by Date + Peptide (Protocol Agnostic)
    ...
    if (!doseAmount) {
         // Fallback to active protocol default
