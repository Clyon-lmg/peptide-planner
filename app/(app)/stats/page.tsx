...
export default async function StatsPage() {
  ...
  // 1. Fetch Doses (Taken only, with date_for)
  ...
  // 2. Fetch Active Protocols (to know which peptides to show)
  ...
  // 3. Fetch Peptides
  const { data: allPeptides } = await supabase.from("peptides").select("*");
